...
( function( wp, $ ) {
	...
	wp.customize( 'custom_logo', function( setting ) {
		setting.bind( function( value ) { 
			if ( '' !== value ) {
				...
				wp.customize.instance( 'gt_office_theme_options[retina_logo]' ).set( false );
			}
		});
		...
		wp.customize.control( 'gt_office_theme_options[retina_logo_title]', setupControl );
		wp.customize.control( 'gt_office_theme_options[retina_logo]', setupControl );
	} );

	// Based on https://make.xwp.co/2016/07/24/dependently-contextual-customizer-controls/
	wp.customize( 'gt_office_theme_options[blog_content]', function( setting ) {
		var setupControl = function( control ) {
			var setActiveState, isDisplayed;
			isDisplayed = function() {
		...
		wp.customize.control( 'gt_office_theme_options[excerpt_length]', setupControl );
	} );
	 ...
	 */
	wp.customize.controlConstructor.gt_office_custom_font = wp.customize.Control.extend({
		/**
	 ...
	 * where type == the type member of a WP_Customize_Control
	 */
	wp.customize.controlConstructor.gt_office_license_key = wp.customize.Control.extend({
		/**
		 * This method is called when the control is ready to run.
		 */
		ready: function() {
