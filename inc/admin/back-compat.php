 ...
 *
 * Prevents GT Office from running on WordPress versions prior to 5.3,
 ...
 * GT Office on WordPress versions prior to 5.3.
 *
 * @global string $wp_version WordPress version.
 */
function gt_office_upgrade_notice() {
	$message = sprintf( esc_html__( '%1$s requires at least WordPress version %2$s. You are running version %3$s. Please upgrade and try again.', 'gt-office' ), 'GT Office', '5.3', $GLOBALS['wp_version'] );
...
function gt_office_customize() {
	wp_die( sprintf( esc_html__( '%1$s requires at least WordPress version %2$s. You are running version %3$s. Please upgrade and try again.', 'gt-office' ), 'GT Office', '5.3', $GLOBALS['wp_version'] ), '', array(
		'back_link' => true,
	) );
}
...
function gt_office_preview() {
	if ( isset( $_GET['preview'] ) ) {
		wp_die( sprintf( esc_html__( '%1$s requires at least WordPress version %2$s. You are running version %3$s. Please upgrade and try again.', 'gt-office' ), 'GT Office', '5.3', $GLOBALS['wp_version'] ) );
	}
}
