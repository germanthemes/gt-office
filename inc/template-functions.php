 ...
 * @return array
 */
function gt_office_body_classes( $classes ) {
...
add_filter( 'body_class', 'gt_office_body_classes' );
 ...
 * @return bool
 */
function gt_office_is_blog_page() {
 ...
 * @return bool
 */
function gt_office_has_sidebar() {
	if ( ! is_active_sidebar( 'sidebar-1' ) ) {
		return false;
	}

	if ( gt_office_is_blog_page() && ! is_page_template( 'templates/template-no-sidebar.php' ) ) {
		return true;
	}
 ...
 * @return void
 */
function gt_office_retina_logo() {
	// Return early if there is no logo image or option for retina logo is disabled.
	if ( ! has_custom_logo() or false === gt_office_get_option( 'retina_logo' ) ) {
		return;
	}
	...
	// Add Custom CSS.
	wp_add_inline_style( 'gt-office-stylesheet', $css );
}
add_filter( 'wp_enqueue_scripts', 'gt_office_retina_logo', 11 );
...
add_filter( 'nav_menu_item_title', 'gt_office_dropdown_icon_to_menu_link', 10, 4 );
 ...
 */
function gt_office_get_svg( $icon = null ) {
 ...
 * @return string $svg SVG markup.
 */
function gt_office_get_social_svg( $icon = null ) {
	// Return early if no icon was defined.
	if ( empty( $icon ) ) {
		return;
	}
 ...
 */
function gt_office_social_icons_menu_walker( $item_output, $item, $depth, $args ) {

	// Get supported social icons.
	$social_icons = gt_office_supported_social_icons();
	...
	if ( 'social-header' === $args->theme_location || 'social-footer' === $args->theme_location ) {
		...
		$item_output = str_replace( $args->link_after, '</span>' . gt_office_get_social_svg( $icon ), $item_output );
	}

	return $item_output;
}
add_filter( 'walker_nav_menu_start_el', 'gt_office_social_icons_menu_walker', 10, 4 );
 ...
 */
function gt_office_supported_social_icons() {
